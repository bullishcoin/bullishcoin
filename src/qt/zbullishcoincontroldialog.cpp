...
#include "ui_zbullishcoincontroldialog.h"
...
std::list<std::string> ZBullishcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBullishcoinControlDialog::listMints;

ZBullishcoinControlDialog::ZBullishcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBullishcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBullishcoinControlDialog::~ZBullishcoinControlDialog()
{
    delete ui;
}

void ZBullishcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBullishcoinControlDialog::updateList()
{
...
void ZBullishcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBullishcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBullishcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBullishcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBullishcoinControlDialog::GetSelectedMints()
{
...
void ZBullishcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
