...
#include "zbullishcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBUL ought to be enough for anybody." - Bill Gates, 2017
    ui->zBULpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBULSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBUL->setEnabled(false);
        ui->pushButtonMintzBUL->setToolTip(tr("zBUL is currently disabled due to maintenance."));

        ui->pushButtonSpendzBUL->setEnabled(false);
        ui->pushButtonSpendzBUL->setToolTip(tr("zBUL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBULpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBUL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBUL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBUL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBUL in ") +
...
void PrivacyDialog::on_pushButtonSpendzBUL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBUL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBUL
        sendzBUL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBUL
    sendzBUL();
}

void PrivacyDialog::on_pushButtonZBullishcoinControl_clicked()
{
    ZBullishcoinControlDialog* zBULControl = new ZBullishcoinControlDialog(this);
    zBULControl->setModel(walletModel);
    zBULControl->exec();
}

void PrivacyDialog::setZBullishcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBULSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBUL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bullishcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBULpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBUL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BULLISHCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBUL</b>";
    ...
    // use mints from zBUL selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBullishcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBullishcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbullishcoin selector in case it was used
    ZBullishcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBUL Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bullishcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBUL Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBUL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBUL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBUL "));
    ui->labelzBULAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBULSyncStatus->setVisible(fShow);
}
